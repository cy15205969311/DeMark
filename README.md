...
- 支持创客贴多页设计稿提取
- 支持 Canva 基础提取流程
- 支持稿定设计分享链接识别、候选原图优选、无水印直链清洗与套图提取
- 支持花瓣 `pin / board / discovery` 公开接口提取、预览图下载与受限状态标注
...
- 对分享页优先识别 `preview_info`、`source_preview_info` 和 `extends_previews`，可直接拼装整套多页结果
...
- 详细实现说明见 [稿定设计提取逻辑说明.md](./稿定设计提取逻辑说明.md)

### Huaban / 花瓣

- 支持 `huaban.com` / `api.huaban.com` / `huabanimg.com` 链接识别与分享文案清洗
- 优先使用花瓣公开 JSON 接口，不经过第三方网关
- 支持 `pin / board / discovery` 三类链接
- 统一把 `file.key` 规范成 `https://hbimg.huabanimg.com/{key}` 形式，方便校验和下载
- 对“只能拿到公开预览图”的商用素材，返回 `preview_image` 警告态，允许继续下载预览图
- 对“连公开预览图都不可用”的受限素材，返回 `preview_only` 警告态，并保留原页跳转
- GUI 中会明确区分普通成功结果、可下载公开预览图但可能有水印的结果、以及仅能提示受限的结果
- 详细实现说明见 [花瓣素材提取逻辑说明.md](./花瓣素材提取逻辑说明.md)
...
├── 稿定设计提取逻辑说明.md        稿定设计提取实现说明
├── 花瓣素材提取逻辑说明.md        花瓣素材提取实现说明
├── main.py                      GUI 启动入口
└── README.md
```
...
- [crawlers/huaban_crawler.py](./crawlers/huaban_crawler.py)：花瓣公开接口提取、预览图 / 受限状态判定逻辑
- [utils/url_parser.py](./utils/url_parser.py)：分享链接清洗、平台识别与稿定 / 花瓣链接整理
- [utils/image_validator.py](./utils/image_validator.py)：图片有效性校验与跨平台防盗链请求头
- [utils/downloader.py](./utils/downloader.py)：单图与整套下载
- [稿定设计提取逻辑说明.md](./稿定设计提取逻辑说明.md)：稿定设计提取实现、分页规则与调试说明
- [花瓣素材提取逻辑说明.md](./花瓣素材提取逻辑说明.md)：花瓣公开接口提取、预览图状态与调试说明
...
- [稿定设计提取逻辑说明.md](./稿定设计提取逻辑说明.md)：稿定提取流程、分页来源、候选筛选与扩展建议

### 花瓣相关文件

- [core/image_extractor.py](./core/image_extractor.py)：把花瓣接入统一调度入口，并对花瓣跳过第三方网关
- [crawlers/huaban_crawler.py](./crawlers/huaban_crawler.py)：花瓣公开接口提取、链接类型识别与 `preview_image / preview_only` 状态构造
- [tests/test_huaban.py](./tests/test_huaban.py)：花瓣链接识别、防盗链请求头、预览图结果与主提取器回归测试
- [花瓣素材提取逻辑说明.md](./花瓣素材提取逻辑说明.md)：花瓣提取流程、状态边界与扩展建议

### 稿定问题排查建议

- 如果提取结果能通过校验但浏览器打开报 OSS 参数错误，优先检查是否带有残缺的 `x-oss-process`
- 如果返回的是带分享文案的链接，优先检查 `utils/url_parser.py` 是否已去掉 `#我分享了...` 这类 fragment
- 如果稿定分享页仍然只返回单图，优先检查分享页里是否存在 `extends_previews`，以及 `GaodingCrawler._extract_preview_bundle_from_html()` 是否命中
...
- 如果日志里显示 `preview_image`，说明当前能拿到公开预览图，但不是无水印原素材
- 如果日志里显示 `preview_only`，说明连公开预览图都没有稳定拿到，优先检查 `hbimg.huabanimg.com` 是否可访问
- 如果花瓣图片校验失败，优先检查 `utils/image_validator.py` 和 `utils/downloader.py` 中的花瓣 `Referer / Origin`
- 如果 board 或 discovery 页结果数量不对，优先检查 `limit` 转发是否正确，以及 pin 列表里是否混入受限商用素材
- 如果用户误以为“预览图下载成功”等于“无水印成功”，优先检查 GUI 中 `warningText` 是否仍然明确

### 推荐验证流程

1. 先运行 `python -m pytest tests/test_gaoding.py -q`，确认 URL 清洗和候选排序没有回退。
2. 再用 GUI 或脚本输入真实稿定分享链接，检查 `processed_url`、最终 `imageUrl`、`imageUrls`、`pageCount` 和下载结果。
3. 如果日志出现 `preview_info.url`、`source_preview_info.url`、`extends_previews`、`export_url` 等字段，可结合日志判断当前命中的候选来源是否合理。
